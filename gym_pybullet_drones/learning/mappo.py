...
def _mlp(input_size, output_size, hidden_size, output_gain):
    layers = []
...
class MAPPO:
    ...
    def __init__(self, observation_space, action_space, config=None, device="cpu"):
        ...
        self.actor = _mlp(self.obs_dim, self.action_dim, self.config.hidden_size, 0.01).to(self.device)
        self.critic = _mlp(self.num_agents * self.obs_dim, 1, self.config.hidden_size, 1.0).to(self.device)
    ...
    def _collect_rollout(self, env, obs, steps):
        ...
        for t in range(steps):
            ...
            values[t] = value
            with torch.no_grad():
                distribution = self._distribution(self._tensor(obs))
    ...
    def _update(self, batch):
        observations, actions, old_log_probs, old_values, advantages, returns = batch
        # A one-step tail has no variance; centering it would erase its signal.
        if len(advantages) > 1:
            advantages = (advantages - advantages.mean()) / (advantages.std(unbiased=False) + 1e-8)
        cfg = self.config
        actor_parameters = list(self.actor.parameters()) + [self.log_std]
        losses = []
        for _ in range(cfg.epochs):
            indices = self.rng.permutation(len(observations))
            for start in range(0, len(indices), cfg.batch_size):
                idx = torch.as_tensor(indices[start:start + cfg.batch_size], device=self.device)
                distribution = self._distribution(observations[idx])
                ...
                # Monte Carlo entropy of the squashed policy, not the base Normal.
    ...
    def save(self, path, metadata=None):
        ...
        torch.save({"config": asdict(self.config), "num_agents": self.num_agents,
                    ...
                    "optimizer": self.optimizer.state_dict(), "num_timesteps": self.num_timesteps,
